...
- [x] Cookie support for gated/private videos, config + UI wiring (upstream issue #43) — turned out to already work with zero code changes, via the existing generic `ydl_options` passthrough (`ydl_options.cookies: /path/to/cookies.txt`). Verified the constructed yt-dlp command includes `--cookies`. Documented in README under a new "Authentication / cookies for gated videos" section, since the gap was purely that this wasn't written down anywhere.
- [x] Detect "video not available yet / scheduled" failures and auto-requeue once the scheduled time passes (upstream issue #146) — `ydlhandler.py` detects this two ways: a regex over the metadata-fetch error text (`will begin in` / `premieres? in`), and `live_status == "is_upcoming"` when metadata fetch itself succeeds. Matching jobs are marked FAILED with `extra_params.not_yet_available = True` instead of raising. `jobshandler.py`'s existing background loop (which already polls its queue every 1s) piggybacks a sweep every 60s that re-queues such jobs after a 30-minute delay, capped at 20 auto-retries. `Logs.vue` shows a distinct "Not available yet / Retry" badge instead of the generic Failed one. Verified live end-to-end: seeded a stale marked job directly in the DB, watched the background sweep pick it up, delete it, re-queue it with an incremented `auto_retry_count`, and complete a real download — with the marker correctly absent from the final successful job.
- [x] Lightweight "check video quality / verify cookies" endpoint, cheaper than a full metadata fetch (upstream issue #108) — new `POST /api/check`, reuses the same `fetch_metadata` call `/api/metadata` already makes (so cookies etc. apply identically) but returns a small distilled summary (title, uploader, duration, live status, best format) instead of the full formats/thumbnails/subtitles payload. Verified against a real video, a real playlist, and an invalid URL.
- [x] Per-extractor conditional yt-dlp options (different output/flags depending on matched site) (upstream issue #68) — new `extractor_options` config section, keyed by yt-dlp's own `extractor` field (e.g. `youtube`), applied as defaults after metadata fetch (which is when the matched extractor becomes known) and merged underneath - never overriding - the format/profile/alias resolution that already happened. Verified live: configured `extractor_options.youtube.write-thumbnail: true` and confirmed a real download produced a `.webp` thumbnail the request itself never asked for.
